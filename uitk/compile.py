...
class PrecompileJob:
    """Handle for an in-flight (or no-op) :meth:`UiCompiler.precompile_async` call.
...
class _UiCompilerInternal(object):
    """Private helpers and mutable state backing :class:`UiCompiler`.

    Kept off the public class so the module's public surface is exactly the
    ``UiCompiler`` staticmethods (plus ``PrecompileJob``). Intra-module
    references are fully qualified (``_UiCompilerInternal._helper`` /
    ``UiCompiler.public``) so the two-class split stays explicit.
    """
    ...
    def _detect_uic_command() -> Tuple[str, list]:
        ...
        bundled = _UiCompilerInternal._find_bundled_uic(binding)
    ...
    def _detect_binding() -> Tuple[str, str]:
        ...
        :meth:`_detect_uic_command` to get the full argv prefix.
        """
        binding, argv = _UiCompilerInternal._detect_uic_command()
    ...
    def _rewrite_customwidget_imports(source: str, customwidgets, resolver) -> str:
        ...
        for class_name, original_header in customwidgets:
            ...
            source = pattern.sub(
                rf"\1from {resolved} import {class_name}", source, count=1
            )
        return source

    @staticmethod
    def _build_header(
        ui_path: Path, binding: str, metadata: dict, source_hash: str
    ) -> str:
        return (
    ...
    def _compile_one(target: Path) -> Tuple[Path, Optional[str]]:
        ...
        try:
            UiCompiler.compile_ui(target, UiCompiler.compiled_path_for(target))
            return target, None
    ...
    def _gather_stale(paths: Iterable[Union[str, Path]]) -> List[Path]:
        ...
        return [
            t
            for t in _UiCompilerInternal._gather_ui_files(paths)
            if not UiCompiler.is_compiled_fresh(t, UiCompiler.compiled_path_for(t))
        ]
    ...
    def _precompile_run(targets: List[Path], jobs: int) -> None:
        ...
        if jobs <= 1 or len(targets) == 1:
            for t in targets:
                _, err = _UiCompilerInternal._compile_one(t)
                if err:
                    logger.warning("uitk precompile: %s failed: %s", t.name, err)
            return
        with ThreadPoolExecutor(max_workers=jobs) as pool:
            for t, err in pool.map(_UiCompilerInternal._compile_one, targets):
                if err:
                    logger.warning("uitk precompile: %s failed: %s", t.name, err)
...
class UiCompiler(_UiCompilerInternal):
    ...
    Public surface for the compiler; all entry points are staticmethods so
    callers use ``UiCompiler.compile_ui(...)`` etc. without instantiating.
    Private helpers live on :class:`_UiCompilerInternal`.
    """
    ...
    def read_embedded_hash(py_path) -> Optional[str]:
        ...
        return _UiCompilerInternal._read_header(py_path, _HASH_RE)

    @staticmethod
    def read_embedded_tags(py_path) -> set:
        """Return __uitk_tags__ from a generated _ui.py header. Empty set if absent."""
        raw = _UiCompilerInternal._read_header(py_path, _TAGS_RE)
        if not raw:
            return set()
        try:
    ...
    def read_embedded_base_class(py_path) -> Optional[str]:
        ...
        return _UiCompilerInternal._read_header(py_path, _BASE_RE)

    @staticmethod
    def read_embedded_form_class(py_path) -> Optional[str]:
        """Return __form_class__ from a generated _ui.py header, or None."""
        return _UiCompilerInternal._read_header(py_path, _FORM_RE)
    ...
    def is_compiled_fresh(ui_path, py_path=None) -> bool:
        ...
        """
        py_path = Path(py_path) if py_path else UiCompiler.compiled_path_for(ui_path)
        if not py_path.exists():
            return False
        embedded = UiCompiler.read_embedded_hash(py_path)
        if embedded is None:
        ...
        return embedded == UiCompiler.hash_ui_source(ui_path)
    ...
    def extract_metadata(ui_path) -> dict:
        ...
        form_class = (
            form_class_el.text if form_class_el is not None else None
        ) or "Form"

        widget_el = root.find("widget")
        base_class = (
            widget_el.get("class") if widget_el is not None else None
        ) or "QWidget"
    ...
    def compile_ui(ui_path, out_path=None, header_resolver=None) -> Path:
        ...
        out_path = Path(out_path) if out_path else UiCompiler.compiled_path_for(ui_path)

        binding, argv_prefix = _UiCompilerInternal._detect_uic_command()
        proc = subprocess.run(
        ...
        body = _UiCompilerInternal._rewrite_imports_to_qtpy(proc.stdout)

        metadata = UiCompiler.extract_metadata(ui_path)

        if header_resolver is not None and metadata["customwidgets"]:
            body = _UiCompilerInternal._rewrite_customwidget_imports(
                body, metadata["customwidgets"], header_resolver
            )

        header = _UiCompilerInternal._build_header(
            ui_path, binding, metadata, UiCompiler.hash_ui_source(ui_path)
        )
    ...
    def ensure_compiled(ui_path, header_resolver=None) -> Path:
        ...
        See :meth:`compile_ui` for the semantics of ``header_resolver``.
        """
        ui_path = Path(ui_path)
        py_path = UiCompiler.compiled_path_for(ui_path)
        if not UiCompiler.is_compiled_fresh(ui_path, py_path):
            UiCompiler.compile_ui(ui_path, py_path, header_resolver=header_resolver)
        return py_path
    ...
    def precompile_async(
        ...
        *paths* regardless of staleness — useful when the user explicitly
        ...
        targets = (
            _UiCompilerInternal._gather_ui_files(paths)
            if force
            else _UiCompilerInternal._gather_stale(paths)
        )
        if not targets:
            return PrecompileJob(thread=None, stale=0, reason="none-stale")
        with _UiCompilerInternal._precompile_lock:
            active = _UiCompilerInternal._precompile_active
            if active is not None and active.is_alive():
                return PrecompileJob(thread=None, stale=len(targets), reason="running")
            thread = threading.Thread(
                target=_UiCompilerInternal._precompile_run,
                args=(targets, workers),
                daemon=True,
                name="uitk-precompile",
            )
            _UiCompilerInternal._precompile_active = thread
            thread.start()
    ...
    def main():
        ...
        todo = [
            ...
            for t in targets
            if args.force
            or not UiCompiler.is_compiled_fresh(t, UiCompiler.compiled_path_for(t))
        ]
        ...
        if args.jobs <= 1 or len(todo) == 1:
            for target in todo:
                t, err = _UiCompilerInternal._compile_one(target)
                if err:
                    print(f"FAILED: {t}\n  {err}")
                    failed.append(t)
                else:
            ...
            with ProcessPoolExecutor(max_workers=args.jobs) as pool:
                futures = {
                    pool.submit(_UiCompilerInternal._compile_one, t): t for t in todo
                }
