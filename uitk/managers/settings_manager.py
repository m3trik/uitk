...
class SettingsManager:
    ...
    def __init__(
        ...
        if qsettings:
            ...
            SettingsManager._maybe_migrate_legacy_registry(org, app)
    ...
    def decode_stored_value(value: Any) -> Any:
        """Read-side mirror of :meth:`encode_stored_value`.
    ...
    def setValue(self, key: str, value: Any) -> None:
        ...
        self.settings.setValue(
            self._ns_key(key), SettingsManager.encode_stored_value(value)
        )
    ...
    # ------------------------------------------------------------------
    # Encapsulated legacy-registry migration (formerly module-level)
    # ------------------------------------------------------------------

    @staticmethod
    def _legacy_qsettings_pairs_for(
        new_org: str, new_app: str
    ) -> List[Tuple[str, str]]:
    ...
    def _maybe_migrate_legacy_registry(org: str, app: str) -> None:
        ...
        * On-disk — a ``Meta/MigratedFromLegacy`` marker key is written
        ...
        pairs = SettingsManager._legacy_qsettings_pairs_for(org, app)
        if not pairs:
            return
        ...
        for old_org, old_app in pairs:
            try:
                ...
                _log.debug(
                    "registry migration: drained %s/%s -> %s/%s (%d keys)",
                    old_org,
                    old_app,
                    org,
                    app,
                ...
                _log.warning(
                    "registry migration: %s/%s -> %s/%s failed: %s",
                    old_org,
                    old_app,
                    org,
                    app,
        ...
        try:
            ...
            _log.warning(
                "registry migration: could not write marker for %s/%s: %s", org, app, e
            )
