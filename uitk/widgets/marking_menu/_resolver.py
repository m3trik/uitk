...
Stateless staticmethods on :class:`MenuResolver` that map an input state
...
class MenuResolver:
    ...
    All entry points are staticmethods; no instance is needed. The class is
    purely a namespace so the resolution logic carries no module-level
    functions.
    """
    ...
    def build_state_key(
        ...
        return MenuResolver.normalize_key(parts)
    ...
    def resolve_target_menu(
        *,
        ...
        key = MenuResolver.build_state_key(
            activation_key_str, buttons, modifiers, extra_key
        )
        if key in bindings:
            return bindings[key]

        # 2. Collapse multi-button mask to its priority button.
        if MenuResolver.count_buttons(buttons) > 1:
            pb = MenuResolver.priority_button(buttons)
            single_key = MenuResolver.build_state_key(
                activation_key_str, pb, modifiers, extra_key
            )
        ...
        # 3. Strip modifiers.
        if modifiers:
            no_mod_key = MenuResolver.build_state_key(
                activation_key_str, buttons, 0, extra_key
            )
            if no_mod_key in bindings:
                return bindings[no_mod_key]
            if MenuResolver.count_buttons(buttons) > 1:
                pb = MenuResolver.priority_button(buttons)
                no_mod_pb_key = MenuResolver.build_state_key(
                    activation_key_str, pb, 0, extra_key
                )
    ...
    def parse_binding_keys(raw_bindings: Mapping[str, str]) -> tuple:
        ...
        for raw_key, ui_name in raw_bindings.items():
            ...
            normalized[MenuResolver.normalize_key(parts)] = ui_name
