...
class MainWindow(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fit_to_content_on_show = fit_to_content_on_show
    ...
    def _activate_descendant_layouts(self) -> None:
        """Re-activate every layout in this window's tree so size hints reflect
        the latest content before a resize (:class:`WindowHeight`)."""
        WindowHeight.activate_layouts(self)

    def _content_min_height(self) -> int:
        """The true minimum height this window's content needs."""
        return SizeGripMixin.content_min_height(self)

    def _sync_min_height_to_hint(self, hint: int) -> None:
        """Align ``minimumHeight`` with a freshly-computed layout hint."""
        WindowHeight.sync_min(self, hint)
    ...
    def adjust_height_by(self, delta: int, baseline: Optional[int] = None) -> None:
        ...
        user previously expanded into. Width is preserved; the result is
        clamped to the content's real minimum and maximum.
        ...
        Parameters:
            ...
            baseline: The window height measured BEFORE the caller changed its
                ...
                here instead races Qt's automatic minimum tracking and
                double-counts the delta. :meth:`WindowHeight.adjust_by` has
                the measurement in full.
        """
        WindowHeight.adjust_by(self, delta, baseline)
    ...
    def fit_height_to_content(self) -> None:
        ...
        into. Use :meth:`adjust_height_by` when that preservation matters.
        """
        WindowHeight.fit_to_content(self)
