...
class OptionBoxManager(ptk.LoggingMixin):
    ...
    def add_option(self, option):
        ...
        Returns:
            self: For fluent interface chaining
        """
        # Ensure option box exists - force creation if adding an option
        if not self._option_box:
            ...
            if existing_option_box:
                ...
                # Create new option box - don't pass action_handler since
                # we're adding options as plugins (which create their own buttons)
                ...
                self._option_box = OptionBoxWithOrdering(
                    ...
                    options=[option],  # Pass the option during creation
                )
                self._container = self._option_box.wrap(self._widget)
                # Don't call add_option again since we already passed it during creation
                return self

        # Delegate to the actual option box widget
        if self._option_box:
