...
### DisableOption
A per-widget **bypass-to-default** toggle. When on it snapshots the wrapped
widget's current value, resets it to its default, and greys the widget out;
when off it restores the snapshot and re-enables. The icon goes the project
"error" red while disabled. Non-persistent (each session starts active).

```python
from uitk.widgets.optionBox.options.disable import DisableOption

disable = DisableOption(spin_box)  # default resolved from window.state at click time
option_box = OptionBox(options=[disable])
...
```python
spin_box.option_box.set_disable()                      # auto (window StateManager)
spin_box.option_box.set_disable(reset=my_reset_func)   # explicit reset callable
```
...
supplied. The snapshot is in-memory (session only). The auto reset runs inside
the StateManager's `suppress_save()`, so the *persisted* value stays your real
one — closing while a field is disabled doesn't bury it at its default (the
disable is fully transient, matching the non-persistent toggle). `set_disabled(
value, emit=False)` flips state silently; `toggled(bool)` fires on user clicks
(`True` = now disabled). Use `find_option(DisableOption)` to retrieve it.
