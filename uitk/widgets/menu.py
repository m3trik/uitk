...
class Menu(QtWidgets.QMenu, AttributesMixin):
    ...
    Parameters:
        ...
        min_child_height (int): The minimum height of any child widgets (excluding the 'Apply' button).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.min_child_height = min_child_height
    ...
    def getChildWidgets(self, inc=[], exc=[]) -> list:
        ...
        if any((inc, exc)):
            return [
                ...
                if w.__class__.__base__.__name__ not in exc
                and (
    ...
    def getActionAtIndex(self, index) -> Optional[QtWidgets.QAction]:
        ...
        """
        try:  # slice the actions list to omit the header and any built-in hidden buttons (ie. 'apply', 'toggle_all_button').
    ...
    def getFormLayout(self, key="form") -> QtWidgets.QFormLayout:
        ...
        except (KeyError, AttributeError):  # create a layout at the given key.
    ...
    def _addApplyButton(self):
        ...
        w.released.connect(lambda: self.parent().clicked.emit())
    ...
    def _addToggleAllButton(self):
        ...
        Returns:
            (widget)
        """
        w = QtWidgets.QPushButton("Uncheck All")
        w.setObjectName("toggle_all_button")
    ...
    def add(self, x, label="", checkable_label=False, **kwargs) -> QtWidgets.QWidget:
        """Add items to the QMenu.

        Parameters:
            x (str/obj): The widget to add. ie. 'QLabel', QtWidgets.QLabel, QtWidgets.QLabel()
            label (str): Add a label. (which is actually a check box. by default it is not checkable)
            checkable_label (bool): The label is checkable.

        kwargs:
            along with any built-in attribute, you can also set any custom attributes defined in the AttributesMixin class.
            for example:
                insert_separator (bool): insert a separator before the widget.
                set_layout_direction (str): ie. 'LeftToRight'
                set_alignment (str): ie. 'AlignVCenter'
                set_button_symbols (str): ie. 'PlusMinus'
                set_limits (str): Set the min, max, and step values with a string. ie. '1-100 step.1'
        ...
        try:  # get the widget
            ...
            widget = x()  # if callable(widget) ie. QtWidgets.QAction(self) object.
        except Exception:
            widget = x
        ...
        typ = widget.__class__.__name__

        if typ == "QAction":
            self.insertAction_(widget)  # add action item.

        else:
            if self.menu_type == "form":  # add widgets to the form layout.
                checkbox = QtWidgets.QCheckBox()
                # format the attr name. ie. 'Subdivisions Height' from 'subdivisionsHeight'
                text = "".join([(" " + i if i.isupper() else i) for i in label]).title()
                checkbox.setText(text)
                checkbox.setObjectName(label)
                if not checkable_label:
                    checkbox.setCheckable(False)

                layout = self.getFormLayout()  # get the default form layout.
                layout.addRow(checkbox, widget)
                # add the widget to the childWidgets list.
                self.childWidgets.add(checkbox)
            ...
            # set child height
            if widget.sizeHint().width() > self.sizeHint().width():
                width = (
                    widget.sizeHint().width()
                    if widget.sizeHint().width() > 125
                    else 125
                )
                widget.setMinimumSize(width, widget.sizeHint().height())
            try:
                checkbox.setMinimumSize(
                    checkbox.sizeHint().width(), self.min_child_height
                )
            except UnboundLocalError:  # 'checkbox' does not exist. (not a form menu)
                pass

            self.childWidgets.add(widget)  # add the widget to the childWidgets list.
            # add the widget's objectName as a QMenu attribute.
            setattr(self, widget.objectName(), widget)

            self._addToContextMenuToolTip(widget)

            if hasattr(widget, "released"):  # Get the appropriate signal to connect to.
                # Connect the signal if one was found.
                widget.released.connect(lambda w=widget: self._setLastActiveChild(w))
            elif hasattr(widget, "valueChanged"):
                widget.valueChanged.connect(
                    lambda value, w=widget: self._setLastActiveChild(value, w)
                )
    ...
    def lastActiveChild(self, name=False, as_list=False):
        ...
        ex. slot connection to the last active child widget:
            cmb.returnPressed.connect(lambda m=cmb.ctx_menu.lastActiveChild: getattr(self, m(name=1))()) #connect to the last pressed child widget's corresponding method after return pressed. ie. self.lbl000 if cmb.lbl000 was clicked last.
        """
        if not hasattr(self, "_lastActiveChild"):
            return None
    ...
    def _addToContextMenuToolTip(self, menuItem) -> None:
        ...
        if not hasattr(self, "_contextMenuToolTip"):
            ...
            p.setToolTip("{}<br><br>{}".format(p.toolTip(), self._contextMenuToolTip))
    ...
    def showEvent(self, event) -> None:
        """ """
        self.resize(self.sizeHint().width(), self.sizeHint().height() + 10)
        ...
        elif isinstance(self.position, (tuple, list, set, QtCore.QPoint)):
        ...
        # if a widget is passed to 'position' (move to the widget's position).
        elif not isinstance(self.position, (type(None), str)):
            ...
            if self.getChildWidgets(inc=["QCheckBox"]):
...
class MenuInstance:
    ...
    def option_menu(self) -> QtWidgets.QMenu:
        ...
        try:
            ...
            self._option_menu = Menu(self, position="bottomLeft", menu_type="standard")
            return self._option_menu
    ...
    def ctx_menu(self) -> QtWidgets.QMenu:
        ...
        try:
            ...
            self._ctx_menu = Menu(self, position="cursorPos", menu_type="context")
            return self._ctx_menu
...
if __name__ == "__main__":
    ...
    # # form layout example
    menu = Menu(menu_type="form", padding=6, title="Drag Me", position="cursorPos")
    s = menu.add("QDoubleSpinBox", label="attrOne", set_spinbox_by_value=1.0)
    s = menu.add("QDoubleSpinBox", label="attrTwo", set_spinbox_by_value=2.0)
    menu.show()
