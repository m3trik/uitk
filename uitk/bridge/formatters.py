...
Each :class:`Formatters` staticmethod has the signature
``formatter(spec, value) -> str`` and renders *value* as a literal in the
target language. Callers feed one of these into
:meth:`uitk.bridge.parameters.Parameters.render_context` to produce the
substitution dict that gets passed to
:meth:`pythontk.str_utils.StrUtils.replace_delimited`.
...
class _FormattersInternal(object):
    """Internal helpers for :class:`Formatters`."""
...
class Formatters(_FormattersInternal):
    """Value formatters that render an :class:`AttributeSpec` value as a
    literal in a target language (Python / Lua / JS / raw argv)."""
    ...
    def python_literal(spec, value: Any) -> str:
        ...
        if isinstance(value, float):
            return _FormattersInternal._strip_trailing_zeros(spec, value)
        return str(value)
