...
- [`managers/reset_gesture.py`](#managers--reset_gesture) — ``ResetGesture`` — the click grammar every *Restore Defaults* control shares.
...
- **[`class Formatters(_FormattersInternal)`](uitk/uitk/bridge/formatters.py#L35)** — Value formatters that render an :class:`AttributeSpec` value as a
  - `Formatters.python_literal(spec, value: Any) -> str` *(static)* — Render *value* as a Python source literal.
  - `Formatters.lua_literal(spec, value: Any) -> str` *(static)* — Render *value* as a Lua source literal.
  - `Formatters.js_literal(spec, value: Any) -> str` *(static)* — Render *value* as a JavaScript literal.
  - `Formatters.cli_raw(spec, value: Any) -> str` *(static)* — Render *value* as a raw command-line argv token (no quoting).
...
- **[`class BridgeSlotsBase(_BridgeSlotsInternal)`](uitk/uitk/bridge/slots.py#L110)** — Base class for DCC-bridge slot panels.
...
- **[`class KindHandler`](uitk/uitk/bridge/spec.py#L157)** — Bundle of callables that build / read / write a widget kind.
- **[`class KindFactory(_KindFactoryInternal)`](uitk/uitk/bridge/spec.py#L1028)** — Build / read / write Qt widgets by ``kind``, backed by the registry.
...
- **[`class UiCompiler(_UiCompilerInternal)`](uitk/uitk/compile.py#L302)** — Compile Qt Designer .ui files to switchboard-augmented _ui.py modules.
  - `UiCompiler.hash_ui_source(ui_path) -> str` *(static)* — SHA-256 hex digest of the .ui file bytes.
  - `UiCompiler.compiled_path_for(ui_path) -> Path` *(static)* — Return the _ui.py path paired with a given .ui path.
  - `UiCompiler.read_embedded_hash(py_path) -> Optional[str]` *(static)* — Return __source_hash__ from a generated _ui.py header, or None.
  - `UiCompiler.read_embedded_tags(py_path) -> set` *(static)* — Return __uitk_tags__ from a generated _ui.py header.
  - `UiCompiler.read_embedded_base_class(py_path) -> Optional[str]` *(static)* — Return __base_class__ from a generated _ui.py header, or None.
  - `UiCompiler.read_embedded_form_class(py_path) -> Optional[str]` *(static)* — Return __form_class__ from a generated _ui.py header, or None.
  - `UiCompiler.is_compiled_fresh(ui_path, py_path=None) -> bool` *(static)* — True only if py_path carries a uitk hash matching ui_path's content.
  - `UiCompiler.extract_metadata(ui_path) -> dict` *(static)* — Extract switchboard-relevant metadata from a .ui file.
  - `UiCompiler.compile_ui(ui_path, out_path=None, header_resolver=None) -> Path` *(static)* — Compile a .ui file to a switchboard-augmented _ui.py.
  - `UiCompiler.ensure_compiled(ui_path, header_resolver=None) -> Path` *(static)* — Return the _ui.py path for ui_path, regenerating if missing or stale.
  - `UiCompiler.precompile_async(*paths: Union[str, Path], jobs: Optional[int] = None, force: bool = False) -> PrecompileJob` *(static)* — Pre-compile _ui.py files in a daemon background thread.
  - `UiCompiler.main()` *(static)* — CLI entry point: python -m uitk.compile [paths...] [--check] [--force] [-j N].

<a id="designer--_designer"></a>
...
- **[`class DesignerPlugin(_DesignerPluginInternal)`](uitk/uitk/designer/_designer.py#L380)** — Registrar that puts uitk widgets in Qt Designer's widget box.
  - `DesignerPlugin.is_design_time() -> bool` *(static)* — Whether this process is Qt Designer loading widgets for a form.
  - `DesignerPlugin.set_design_time(value: bool) -> None` *(static)* — Mark (or unmark) this process as a Qt Designer session.
  - `DesignerPlugin.collect(cls, source='widgets', base_dir: Optional[str] = None, group: Optional[str] = None, recursive: bool = True) -> List[DesignerWidget]` *(class)* — Resolve Designer metadata for every widget class under ``source``.
  - `DesignerPlugin.register(cls, source='widgets', base_dir: Optional[str] = None, group: Optional[str] = None, recursive: bool = True) -> List[DesignerWidget]` *(class)* — Publish the collected widgets to Qt Designer's widget box.
  - `DesignerPlugin.plugin_dirs(cls) -> List[str]` *(class)* — Directories holding a Designer entry file, for ``PYSIDE_DESIGNER_PLUGINS``.
  - `DesignerPlugin.environment(cls, plugin_dirs: Optional[Sequence[str]] = None, python_paths: Optional[Sequence[str]] = None, env: Optional[Dict[str, str]] = None) -> Dict[str, str]` *(class)* — Return an environment mapping that lets Designer import and find uitk.
  - `DesignerPlugin.launch(cls, *ui_files: str, plugin_dirs: Optional[Sequence[str]] = None, python_paths: Optional[Sequence[str]] = None, wait: bool = True) -> int` *(class)* — Start Qt Designer with uitk's widgets available in the widget box.
...
- **[`class CompiledLoader`](uitk/uitk/loaders/compiled.py#L34)** — Switchboard delegate that loads UIs via compiled _ui.py modules.
...
- **[`class RuntimeLoader`](uitk/uitk/loaders/runtime.py#L55)** — Switchboard delegate that loads UIs at runtime via QUiLoader.
...
<a id="managers--color_model"></a>
### `managers/color_model.py`

The observable colour value a picker and its widgets share.

- **[`class ColorModel`](uitk/uitk/managers/color_model.py#L32)** — One observable colour, stored as float HSVA with the authored hue kept.
  - `ColorModel.hsva(self) -> Tuple[float, float, float, float]` *(property)* — ``(h, s, v, a)`` in 0-1 -- the stored value, exactly as authored.
  - `ColorModel.hue(self) -> float` *(property)* — The AUTHORED hue, which survives a trip through black or grey.
  - `ColorModel.saturation(self) -> float` *(property)*
  - `ColorModel.value(self) -> float` *(property)*
  - `ColorModel.alpha(self) -> float` *(property)*
  - `ColorModel.rgbf(self) -> Tuple[float, float, float]` *(property)* — ``(r, g, b)`` in 0-1 -- what a DCC attribute or shader wants.
  - `ColorModel.rgbaf(self) -> Tuple[float, float, float, float]` *(property)*
  - `ColorModel.color(self) -> ptk.Color` *(property)* — The value as a :class:`pythontk.Color` -- the boundary type.
  - `ColorModel.hex(self) -> str` *(property)*
  - `ColorModel.mixed(self) -> bool` *(property)* — Whether this stands for several values that disagree.
  - `ColorModel.set_hsv(self, h: Optional[float] = None, s: Optional[float] = None, v: Optional[float] = None, a: Optional[float] = None, notify: bool = True) -> bool` — Set any subset of the components.
  - `ColorModel.set_rgbf(self, r, g, b, a: Optional[float] = None, notify: bool = True) -> bool` — Set from 0-1 floats, KEEPING the authored hue where rgb cannot say.
  - `ColorModel.set_color(self, value, notify: bool = True) -> bool` — Set from anything :meth:`to_rgbaf` understands.
  - `ColorModel.to_rgbaf(cls, value) -> Optional[Tuple[float, float, float, float]]` *(class)* — Read a colour in any shape this package passes around into 0-1 floats.
  - `ColorModel.set_mixed(self, mixed: bool = True, notify: bool = True) -> bool` — Mark (or clear) the indeterminate state without touching the value.
  - `ColorModel.subscribe(self, callback: Callable[[], None]) -> None` — Register *callback* (no args) to run after any change.
  - `ColorModel.unsubscribe(self, callback: Callable[[], None]) -> None` — Remove a callback (no-op if absent).
  - `ColorModel.muted(self) -> '_Mute'` — Context manager coalescing every change inside it into ONE notify.

<a id="managers--cursor_manager"></a>
### `managers/cursor_manager.py`
...
- **[`class CursorManager`](uitk/uitk/managers/cursor_manager.py#L291)** — Static service for both of Qt's cursor systems — see the module
  ...
  - `CursorManager.drain() -> None` *(static)* — Pop the entire application override-cursor stack, for good.
  - `CursorManager.release(shape, app=None) -> bool` *(static)* — Remove the top-most override entry of ``shape`` — and nothing else.
  - `CursorManager.busy(shape=QtCore.Qt.WaitCursor, suspend_for_modals: bool = True)` *(static)* — Show ``shape`` application-wide for the duration of a ``with`` block.
  - `CursorManager.has_explicit_cursor(target) -> bool` *(static)* — True when ``target`` carries a cursor of its own rather than
  - `CursorManager.push(cls, target, shape) -> None` *(class)* — Show ``shape`` on ``target`` until :meth:`pop` — a gesture cursor.
  - `CursorManager.pop(cls, target) -> bool` *(class)* — Undo the most recent :meth:`push` on ``target``.
  - `CursorManager.heal_hover(widget, global_pos) -> bool` *(static)* — Re-run Qt's enter dispatch for ``widget`` when its hover state is

<a id="managers--field_visibility"></a>
### `managers/field_visibility.py`
...
- **[`class FieldVisibility`](uitk/uitk/managers/field_visibility.py#L12)** — The fields a container shows, as named modes over one keyed registry.
  ...
  - `FieldVisibility.define(self, mode: str, fields: Iterable[Hashable]) -> 'FieldVisibility'` — Store *fields* as the mode called *mode*.
  - `FieldVisibility.bind(self, combo) -> 'FieldVisibility'` — Drive the mode from a combo box's current entry.
  - `FieldVisibility.keys(self) -> tuple` *(property)* — Every registered key -- what this is in a position to hide.
  - `FieldVisibility.mode(self) -> Optional[str]` *(property)* — The named mode currently applied, if it was applied by name.
  - `FieldVisibility.visible(self) -> tuple` *(property)* — The keys currently on screen.
  - `FieldVisibility.show(self, keys: Iterable[Hashable]) -> None` — Show exactly *keys* -- the form for a set nobody stored in advance.
...
- **[`class IconManager`](uitk/uitk/managers/icon_manager.py#L10)** — Theme-aware SVG icon loader with caching and color customization.
  ...
  - `IconManager.set_label_icon(cls, label, name: str, size=(14, 14), color: str = None, auto_theme: bool = True)` *(class)* — Prefix a text-bearing widget (e.g.
...
- **[`class RecentValueEntry`](uitk/uitk/managers/recent_values_store.py#L25)** — A recent value whose restore-data differs from its display string.
- **[`class RecentValuesStore`](uitk/uitk/managers/recent_values_store.py#L60)** — Ordered, deduped, most-recent-first value history.
  - `RecentValuesStore.normalize_value(value)` *(static)* — Normalize a value for comparison.
...
<a id="managers--reset_gesture"></a>
### `managers/reset_gesture.py`
...
- **[`class ResetGesture(QtCore.QObject)`](uitk/uitk/managers/reset_gesture.py#L29)** — Modifier-aware reset for a button: dispatch, tooltip, live preview.
  ...
  - `ResetGesture.tooltip(title: str = 'Restore Defaults', *, saving: bool = True, saved: Optional[bool] = None, bypass: Optional[List[str]] = None) -> str` *(static)* — The rich tooltip that teaches the grammar.
  - `ResetGesture.perform(cls, state, action: str, widgets=None) -> Optional[str]` *(class)* — Apply *action* through *state* over *widgets*.
  - `ResetGesture.trigger(self, *_) -> str` — Run the action the held modifiers select.
  - `ResetGesture.refresh_tooltip(self) -> None` — Rebuild the tooltip for the current state (runs on every hover).
  - `ResetGesture.flash(self, message: str) -> None` — Show *message* on the button for :data:`FLASH_MS`, then its label.
  - `ResetGesture.eventFilter(self, obj, event)`
...
- **[`class SettingsManager`](uitk/uitk/managers/settings_manager.py#L82)** — Manages persistent storage and retrieval of settings via QSettings.
  ...
  - `SettingsManager.decode_stored_value(value: Any) -> Any` *(static)* — Read-side mirror of :meth:`encode_stored_value`.
  - `SettingsManager.encode_stored_value(value: Any) -> Any` *(static)* — Encode *value* for QSettings so a JSON-decoding read restores it losslessly.
...
- **[`class ShortcutManager`](uitk/uitk/managers/shortcut_manager.py#L333)** — Centralized shortcut management with clear separation of concerns
  ...
  - `ShortcutManager.resolve_application_host(widget: Optional[QtWidgets.QWidget]) -> Optional[QtWidgets.QWidget]` *(static)* — Return an always-visible top-level window to own an application shortcut.
  - `ShortcutManager.find_duplicate_application_shortcuts(app=None) -> Dict[str, int]` *(static)* — Return ``{sequence: count}`` for key sequences bound by more than one
...
- **[`class StateManager(ptk.LoggingMixin)`](uitk/uitk/managers/state_manager.py#L13)** — Manages widget state persistence using QSettings.
  ...
  - `StateManager.for_widget(widget: QtWidgets.QWidget) -> Optional['StateManager']` *(static)* — The state manager that owns *widget*, or ``None``.
  - `StateManager.reset_all(self, block_signals: bool = False, widgets=None, factory: bool = False) -> None` — Reset widgets to their defaults (see :meth:`default_for`).
  - `StateManager.reset(self, widget: QtWidgets.QWidget) -> None` — Reset a widget to its default (see :meth:`default_for`).
...
- **[`class WindowHeight`](uitk/uitk/managers/window_height.py#L12)** — One implementation of "my content changed -- follow it".
  - `WindowHeight.activate_layouts(window: QtWidgets.QWidget) -> None` *(static)* — Invalidate + re-activate every layout in *window*'s tree so size
  - `WindowHeight.sync_min(window: QtWidgets.QWidget, hint: int) -> None` *(static)* — Align *window*'s ``minimumHeight`` with a freshly-computed hint.
...
- **[`class SwitchboardShortcutMixin`](uitk/uitk/switchboard/shortcuts.py#L91)** — Mixin for managing keyboard shortcuts for Switchboard Slots.
  ...
  - `SwitchboardShortcutMixin.widget_has_shortcut(self, widget: QtWidgets.QWidget) -> bool` — Whether *widget*'s slot currently holds a bound keyboard shortcut.
...
- **[`class SwitchboardWidgetMixin`](uitk/uitk/switchboard/widgets.py#L9)** — Widget registration, resolution, and dynamic class loading for Switchboard.
  - `SwitchboardWidgetMixin.unmet_policy(self) -> str` *(property)* — How tools whose requirement is unmet are presented (persisted).
  - `SwitchboardWidgetMixin.gate(self, widget, available, reason: str = '') -> bool` — Present *widget* according to whether its requirement is met.
  - `SwitchboardWidgetMixin.recheck_gates(self) -> int` — Re-apply every live gate;
...
- **[`class ColorSwatch(QtWidgets.QPushButton, AttributesMixin, ConvertMixin)`](uitk/uitk/widgets/colorSwatch.py#L9)** — Color picker button that displays and stores a selectable color value.
  ...
  - `ColorSwatch.setKeepSquare(self, value: bool) -> None` — Set aspect locking (Qt-property setter for :attr:`keep_square`).
...
- **[`class ComboBox(AlignedComboBox, MenuMixin, OptionBoxMixin, AttributesMixin, RichText, TextOverlay)`](uitk/uitk/widgets/comboBox.py#L600)** — QComboBox with automatic Menu and OptionBox integration.
  ...
  - `ComboBox.setCurrentTextPrefix(self, value: str) -> None` — Qt-property setter for :attr:`current_text_prefix`.
  - `ComboBox.setCurrentTextSuffix(self, value: str) -> None` — Qt-property setter for :attr:`current_text_suffix`.
...
- **[`class CenteredIconActionDelegate(RowSelectionBorderDelegate)`](uitk/uitk/widgets/delegates/centered_icon.py#L43)** — Centers an item's icon while preserving the row-selection border.
  ...
  - `CenteredIconActionDelegate.paint_centered_icon(painter, icon, cell, decoration_size, hover=False, opacity=1.0)` *(static)* — Draw *icon* centered within the *cell* rect.
...
- **[`class ChoiceCaptureDelegate(QtWidgets.QStyledItemDelegate)`](uitk/uitk/widgets/delegates/choice_capture.py#L37)** — Item delegate that edits a cell via an in-cell dropdown.
  ...
  - `ChoiceCaptureDelegate.install_choice_capture(table: QtWidgets.QTableWidget, column: int, choices: Iterable[str], on_capture, *, editable: bool = True, bordered: bool = False) -> ChoiceCaptureDelegate` *(static)* — Wire in-cell dropdown capture onto a table column.
- **[`class BorderedChoiceCaptureDelegate(ChoiceCaptureDelegate, RowSelectionBorderDelegate)`](uitk/uitk/widgets/delegates/choice_capture.py#L166)** — :class:`ChoiceCaptureDelegate` that paints the row-spanning
...
- **[`class ShortcutCaptureEdit(QtWidgets.QLineEdit)`](uitk/uitk/widgets/delegates/shortcut_capture.py#L34)** — Read-only line edit that captures a single key chord.
...
- **[`class ShortcutCaptureDelegate(QtWidgets.QStyledItemDelegate)`](uitk/uitk/widgets/delegates/shortcut_capture.py#L109)** — Item delegate that edits a cell via in-cell key capture.
  ...
  - `ShortcutCaptureDelegate.install_shortcut_capture(table: QtWidgets.QTableWidget, column: int, on_capture, *, bordered: bool = False) -> ShortcutCaptureDelegate` *(static)* — Wire in-cell shortcut capture onto a table column.
- **[`class BorderedShortcutCaptureDelegate(ShortcutCaptureDelegate, RowSelectionBorderDelegate)`](uitk/uitk/widgets/delegates/shortcut_capture.py#L196)** — :class:`ShortcutCaptureDelegate` that paints the row-spanning
...
<a id="widgets--editors--color_editor"></a>
### `widgets/editors/color_editor.py`
...
- **[`class ColorEditor(QtWidgets.QWidget, AttributesMixin)`](uitk/uitk/widgets/editors/color_editor.py#L37)** — A colour editor built from named sections, bound to one shared model.
  ...
  - `ColorEditor.set_mixed(self, mixed: bool = True) -> None` — Show as indeterminate, for a selection whose colours disagree.
...
- **[`class RampPreview(QtWidgets.QFrame)`](uitk/uitk/widgets/editors/color_editor.py#L606)** — Animates what a keyed channel does, as a flat 2D fill.
  ...
  - `RampPreview.waveform(self)` *(property)* — How this preview shapes time.
  - `RampPreview.elapsed(self) -> float` *(property)* — Seconds into the cycle, read off a clock shared by every preview.
  - `RampPreview.set_base(self, rgb) -> None` — The material's own emissive the channel composes OVER.
  - `RampPreview.sample(self, seconds: float) -> float` — The channel's 0-1 value at *seconds* in, per this waveform.
  - `RampPreview.composite(self, sample: float)` — The components the channel would carry at this *sample*.
  - `RampPreview.display(self, sample: float)` — ``(r, g, b, a)`` as PAINTED for *sample* -- see :meth:`composite`.
...
- **[`class ColorRampEditor(QtWidgets.QWidget)`](uitk/uitk/widgets/editors/color_editor.py#L811)** — One :class:`ColorEditor` per named end of a ramp, side by side.
  ...
  - `ColorRampEditor.editors(self) -> tuple` *(property)*
  - `ColorRampEditor.editor(self, which) -> ColorEditor` — The editor at an index, or under a label (case-insensitive).
  - `ColorRampEditor.colors(self) -> tuple` *(property)* — The DISPLAYED colours, ``(pythontk.Color, ...)`` in label order.
...
- **[`class ExpandableList(QtWidgets.QWidget, AttributesMixin)`](uitk/uitk/widgets/expandableList.py#L68)** — A subclass of QWidget that represents a list of widgets, each potentially having an expandable subl…
  ...
  - `ExpandableList.setMaxItemHeight(self, value: int) -> None` — Set the per-item maximum height;
...
- **[`class Footer(QtWidgets.QWidget, AttributesMixin, SizeGripMixin)`](uitk/uitk/widgets/footer.py#L19)** — Footer is a widget that acts as a status bar with an integrated
  ...
  - `Footer.getDefaultStatusText(self) -> str` — Get the fallback text shown when no explicit status is set.
  - `Footer.getSizeGripEnabled(self) -> bool` — Whether the footer currently shows a resize grip.
  - `Footer.setSizeGripEnabled(self, value: bool) -> None` — Show or hide the resize grip, creating it on first enable.
  - `Footer.setStatus(self, value: str) -> None` — Set the status text (Qt-property setter for :meth:`setStatusText`).
  - `Footer.start_progress(self, total: Optional[int] = None, text: str = '', busy: Optional[bool] = None) -> Callable[[Optional[int], Optional[str]], bool]` — Start showing progress in the footer.
  ...
  - `Footer.progress(self, total: Optional[int] = None, text: str = '', busy: Optional[bool] = None) -> 'FooterProgressContext'` — Context manager for cooperative progress / task feedback.
  - `Footer.busy_indicator(self) -> QtWidgets.QLabel` *(property)* — The spinner label shown beside the status text while busy.
  - `Footer.is_busy(self) -> bool` *(property)* — Whether the busy indicator is showing.
  - `Footer.set_busy(self, busy: bool) -> None` — Show or hide the busy spinner beside the status text.
...
- **[`class FooterProgressContext`](uitk/uitk/widgets/footer.py#L960)** — Context manager for footer progress tracking.
- **[`class FooterStatusController`](uitk/uitk/widgets/footer.py#L988)** — Helper that keeps a footer in sync with a resolver function.
...
- **[`class FormPanel(WindowPanel)`](uitk/uitk/widgets/formPanel.py#L46)** — Themed form window over a list of field specs.
...
- [`STEPS`](uitk/uitk/widgets/gradient_slider.py#L16) — constant
- **[`class GradientSlider(Slider)`](uitk/uitk/widgets/gradient_slider.py#L19)** — A :class:`Slider` whose groove is the live ramp of one colour channel.
  - `GradientSlider.channel(self) -> str` *(property)*
  - `GradientSlider.model(self) -> Optional[ColorModel]` *(property)*
  - `GradientSlider.bind(self, model: ColorModel) -> None` — Drive (and be driven by) *model*, releasing any previous one.
  - `GradientSlider.refresh(self) -> None` — Re-read the model: move the handle and redraw the track.
  - `GradientSlider.event(self, event)`

<a id="widgets--header"></a>
### `widgets/header.py`

- **[`class Header(QtWidgets.QLabel, AttributesMixin, RichText, TextOverlay, ptk.LoggingMixin)`](uitk/uitk/widgets/header.py#L14)** — Header is a QLabel that can be dragged around the screen and can be pinned/unpinned.
  ...
  - `Header.setHelpText(self, value: str) -> None` — Set the help text (Qt-property setter for :meth:`set_help_text`).
  - `Header.setPinOnDragOnly(self, value: bool) -> None` — Set the pin-click mode (Qt-property setter for the attribute).
  - `Header.getConfigButtons(self) -> str` — Return the configured header buttons as a space-separated string.
  - `Header.setConfigButtons(self, value: str) -> None` — Configure header buttons from a comma- or space-separated string.
  - `Header.getAutoHideWithOsFrame(self) -> bool` — Whether the header hides itself beside a native OS title bar.
  - `Header.setAutoHideWithOsFrame(self, value: bool) -> None` — Set whether the header hides itself beside a native OS title bar.
...
- **[`class MainWindow(QtWidgets.QMainWindow, AttributesMixin, TooltipMixin, ptk.LoggingMixin)`](uitk/uitk/widgets/mainWindow.py#L21)** — Application main window with state persistence and child widget management.
...
- **[`class MenuResolver`](uitk/uitk/widgets/marking_menu/_resolver.py#L27)** — Pure, stateless menu-resolution primitives for the MarkingMenu.
  ...
  - `MenuResolver.count_buttons(buttons: int) -> int` *(static)* — Count distinct buttons set in the mask.
  - `MenuResolver.resolve_target_menu(*, activation_held: bool, activation_key_str: Optional[str], buttons: int, modifiers: int, bindings: Mapping[str, str], extra_key: Optional[str] = None) -> Optional[str]` *(static)* — Return the UI name that should be visible for the given input state.
  - `MenuResolver.parse_binding_keys(raw_bindings: Mapping[str, str]) -> tuple` *(static)* — Parse user-supplied bindings into (normalized_dict, activation_key_str).
...
- **[`class MenuConfig`](uitk/uitk/widgets/menu.py#L76)** — Configuration for Menu initialization.
...
- **[`class ActionButtonManager`](uitk/uitk/widgets/menu.py#L164)** — Manages action buttons for Menu widgets.
...
- **[`class MenuPositioner`](uitk/uitk/widgets/menu.py#L340)** — Encapsulates menu positioning and width matching logic.
...
- **[`class Menu(QtWidgets.QWidget, AttributesMixin, ptk.LoggingMixin)`](uitk/uitk/widgets/menu.py#L577)** — A custom Qt Widget that serves as a popup menu with additional features.
...
- **[`class MessageBox(QtWidgets.QMessageBox, AttributesMixin)`](uitk/uitk/widgets/messageBox.py#L8)** — Displays a message box with HTML formatting for a set time before closing.
  ...
  - `MessageBox.as_prompt(self)` — Re-flag this box as an interactive prompt rather than a toast.
  - `MessageBox.exec_(self)`

<a id="widgets--mixins--attributes"></a>
### `widgets/mixins/attributes.py`

- **[`class AttributesMixin`](uitk/uitk/widgets/mixins/attributes.py#L12)** — A mixin class providing a comprehensive interface for setting attributes on Qt widgets.
  - `AttributesMixin.is_design_time() -> bool` *(static)* — Whether this widget is being built by Qt Designer rather than an app.
  - `AttributesMixin.set_design_time(value: bool) -> None` *(static)* — Mark (or unmark) this process as a Qt Designer session.
...
- **[`class TooltipProxy(TooltipFormat, _TooltipBindInternal)`](uitk/uitk/widgets/mixins/tooltip_mixin.py#L528)** — Per-widget tooltip namespace stamped on each registered MainWindow widget.
  - `TooltipProxy.bind(self, provider) -> None` — Register a callable() -> str called lazily on QEvent.ToolTip hover.
- **[`class TooltipNamespace(TooltipFormat, _TooltipBindInternal)`](uitk/uitk/widgets/mixins/tooltip_mixin.py#L575)** — The Switchboard's ``sb.tooltip`` namespace — owner of the tooltip surface.
...
- **[`class TooltipMixin`](uitk/uitk/widgets/mixins/tooltip_mixin.py#L634)** — Mixin for MainWindow — stamps ``widget.tooltip`` on every registered widget.
...
- **[`class AffixMode`](uitk/uitk/widgets/optionBox/options/affix.py#L72)** — One selectable state of the picker.
  ...
  - `AffixMode.convention(cls, convention_key: Union[str, Callable[[], str]], *, key: str = 'convention', label: str = 'Scene', icon: str = 'link', description: str = '') -> 'AffixMode'` *(class)* — A mode bound to ``pythontk.NamingConvention`` for *convention_key*.
- **[`class AffixOption(PersistedOption, ButtonOption)`](uitk/uitk/widgets/optionBox/options/affix.py#L232)** — Cycling affix-mode picker for a text widget.
...
- **[`class OverflowIndicator(QtWidgets.QWidget)`](uitk/uitk/widgets/overflow_indicator.py#L10)** — A mouse-transparent overlay on a scroll area's viewport that marks each
  - `OverflowIndicator.attach(cls, area: QtWidgets.QAbstractScrollArea) -> 'OverflowIndicator'` *(class)* — Return *area*'s indicator, creating it on the first call.
  - `OverflowIndicator.of(cls, area: QtWidgets.QAbstractScrollArea) -> Optional['OverflowIndicator']` *(class)* — The indicator attached to *area*, or ``None``.
  - `OverflowIndicator.detach(self) -> None` — Take the overlay off its area and schedule it for deletion.
  - `OverflowIndicator.area(self) -> Optional[QtWidgets.QAbstractScrollArea]` *(property)* — The scroll area this overlays (its parent);
  - `OverflowIndicator.shown_edges(self) -> Tuple[str, ...]` *(property)* — The edges currently marked, in ``("top", "bottom")`` order --
...
- **[`class ProgressTaskContext`](uitk/uitk/widgets/progressBar.py#L515)** — Context manager for progress bar tasks.
...
- **[`class Region(QtWidgets.QWidget, AttributesMixin, ConvertMixin)`](uitk/uitk/widgets/region.py#L8)** — A custom QWidget that represents a region with a specified shape and size.
  ...
  - `Region.setVisibleOnMouseOver(self, value: bool) -> None` — Set reveal-on-hover (Qt-property setter for the attribute above).
...
- **[`class Separator(QtWidgets.QFrame, AttributesMixin)`](uitk/uitk/widgets/separator.py#L10)** — A horizontal rule;
  - `Separator.getTitle(self) -> str` — Get the separator title.
  - `Separator.setTitle(self, value: str) -> None` — Set the separator title.
  - `Separator.isCheckable(self) -> bool` — Whether the caption is a disclosure (see :meth:`setCheckable`).
  - `Separator.setCheckable(self, value: bool) -> None` — Make the caption a disclosure: clickable, a chevron showing its state.
  - `Separator.isChecked(self) -> bool` — Open (``True``) or closed;
  - `Separator.setChecked(self, value: bool) -> None` — Set the disclosure state, emitting :attr:`toggled` on a change.
  - `Separator.toggle(self) -> None`
  - `Separator.mousePressEvent(self, event) -> None`
...
- **[`class FrameTooltip`](uitk/uitk/widgets/sequencer/_drag_tooltip.py#L38)** — Floating ``QGraphicsSimpleTextItem`` that tracks the cursor.
...
- **[`class KeyframeItem(DraggableItemMixin, QtWidgets.QGraphicsEllipseItem)`](uitk/uitk/widgets/sequencer/_keyframe.py#L25)** — An interactive keyframe indicator inside a sub-row :class:`ClipItem`.
  ...
  - `KeyframeItem.weighted_handles(self) -> bool` — Whether a handle's LENGTH carries meaning on this curve.
...
- **[`class TangentHandleItem(QtWidgets.QGraphicsEllipseItem)`](uitk/uitk/widgets/sequencer/_keyframe.py#L552)** — The grab point of a selected key's IN or OUT tangent handle.
  ...
  - `TangentHandleItem.drag_participant(self) -> bool` *(property)* — True while this handle is being dragged, or carried by one that
...
- **[`class RangeHighlightItem(DraggableItemMixin, QtWidgets.QGraphicsItem)`](uitk/uitk/widgets/sequencer/_overlays.py#L565)** — A semi-transparent rectangle highlighting a time range on the timeline.
  ...
  - `RangeHighlightItem.begin_edge_drag(self, edge: str, scene_x: float) -> None` — Start a bound drag from outside the item (the shot lane's handles).
  ...
  - `RangeHighlightItem.update_edge_drag(self, scene_x: float) -> None` — Advance a drag begun with :meth:`begin_edge_drag`.
  - `RangeHighlightItem.finish_edge_drag(self) -> bool` — End the drag and emit if a bound actually moved.
...
- **[`class RulerItem(QtWidgets.QGraphicsItem)`](uitk/uitk/widgets/sequencer/_ruler.py#L36)** — Draws the timeline header: the frame ruler, and the shot lane below it.
  ...
  - `RulerItem.set_key_ticks(self, times) -> None` — Frames that carry a key, drawn as faint red ticks (Maya's idiom).
  - `RulerItem.set_content_left(self, left: float) -> None` — Set the left edge of the extent the ruler covers (scene pixels).
...
- **[`class ScrubPlayer(QtCore.QObject)`](uitk/uitk/widgets/sequencer/_scrub_player.py#L42)** — Seek-and-grain player for NLE-style audio scrub.
...
- **[`class AttributeColorDialog(ColorMappingDialog)`](uitk/uitk/widgets/sequencer/_sequencer.py#L75)** — Dialog for configuring attribute-type color mappings.
...
- **[`class SequencerWidget(QtWidgets.QSplitter, AttributesMixin)`](uitk/uitk/widgets/sequencer/_sequencer.py#L197)** — A split-view NLE sequencer widget.
  ...
  - `SequencerWidget.shortcut_overlay_mode(self) -> str` *(property)* — When to show the legend: ``"off"``, ``"on"``, or ``"modifier"``.
  - `SequencerWidget.shortcut_overlay_tracking(self) -> bool` *(property)* — Whether the group under the pointer is worth resolving: the legend
  ...
  - `SequencerWidget.set_shift_held(self, held: bool) -> None` — Record whether Shift is down -- the one-key face of
  - `SequencerWidget.modifiers_held(self) -> int` *(property)* — The chord currently held over the timeline, as a plain int.
  - `SequencerWidget.set_modifiers_held(self, modifiers) -> None` — Record the chord held over the timeline;
...
- **[`class TrackHeaderWidget(QtWidgets.QWidget)`](uitk/uitk/widgets/sequencer/_timeline.py#L97)** — Left-pane widget showing track labels, vertically synced to the timeline.
  ...
  - `TrackHeaderWidget.selected_sub_rows(self) -> List[tuple]` — ``[(track_name, sub_name), ...]`` for the selected sub-rows.
...
- **[`class PlayController(Protocol)`](uitk/uitk/widgets/sequencer/_transport_controls.py#L32)** — Minimal transport API the controls drive.
  - `PlayController.is_playing(self) -> bool`
  - `PlayController.play(self, forward: bool) -> None`
  - `PlayController.stop(self) -> None`
- **[`class ScrubPlayerPlayController`](uitk/uitk/widgets/sequencer/_transport_controls.py#L40)** — Default :class:`PlayController` backed by the sequencer's ScrubPlayer.
...
- **[`class TransportControls(QtWidgets.QWidget)`](uitk/uitk/widgets/sequencer/_transport_controls.py#L102)** — Maya-style 8-button transport row bound to a :class:`SequencerWidget`.
...
- **[`class TableWidget(QtWidgets.QTableWidget, MenuMixin, HeaderMixin, AttributesMixin, CellFormatMixin)`](uitk/uitk/widgets/tableWidget.py#L567)** — Enhanced QTableWidget with cell formatting, sorting, and context menu support.
  ...
  - `TableWidget.setLeftClickSelectOnly(self, enabled: bool) -> None` — Qt-property setter for :meth:`set_left_click_select_only`.
...
- **[`class TableActions`](uitk/uitk/widgets/table_actions.py#L125)** — Manages action columns on a :class:`TableWidget`.
  - `TableActions.add(self, column: int, states: Dict[str, Dict[str, Any]], header_icon: str | None = None, square: bool = True, drag_action: Optional[Callable[[List[int], int], None]] = None) -> None` — Register an action column.
...
- **[`class HierarchyIconMixin`](uitk/uitk/widgets/treeWidget.py#L16)** — Mixin to handle custom hierarchy icons in tree widgets using CSS branch styling.
...
- **[`class TreeFormatMixin(ConvertMixin)`](uitk/uitk/widgets/treeWidget.py#L188)** — Generic item/column formatting for QTreeWidget.
...
- **[`class TreeWidget(QtWidgets.QTreeWidget, MenuMixin, AttributesMixin, TreeFormatMixin, HierarchyIconMixin)`](uitk/uitk/widgets/treeWidget.py#L583)** — Enhanced QTreeWidget with flexible data handling, formatting capabilities, and custom hierarchy ico…
  ...
  - `TreeWidget.getSelectionStyle(self) -> str` — Visual style for selected rows: ``"border"`` or ``"tint"``.
  - `TreeWidget.setSelectionStyle(self, value: str) -> None` — Set the selected-row style;
  - `TreeWidget.getCtrlToggle(self) -> bool` — Whether Ctrl+click deselects an already-selected item.
  - `TreeWidget.setCtrlToggle(self, value: bool) -> None` — Enable or disable Ctrl+click deselection.
...
- **[`class WidgetComboBox(ComboBox)`](uitk/uitk/widgets/widgetComboBox.py#L135)** — ComboBox extended with widget embedding support.
  ...
  - `WidgetComboBox.row_of(self, widget: QtWidgets.QWidget) -> Optional[int]` — The model row embedding *widget*, or ``None``.
  - `WidgetComboBox.row_container(self, widget: QtWidgets.QWidget) -> Optional[QtWidgets.QWidget]` — The marginless container (caption + widget) of *widget*'s row.
  - `WidgetComboBox.set_row_visible(self, widget: QtWidgets.QWidget, visible: bool) -> None` — Show or hide the row embedding *widget*.
  - `WidgetComboBox.is_row_visible(self, widget: QtWidgets.QWidget) -> bool` — False when *widget*'s row is hidden (or *widget* is not a row).
  - `WidgetComboBox.field_key(self, widget: QtWidgets.QWidget)` — The key *widget*'s row registers under on :attr:`fields`: its
  - `WidgetComboBox.fields(self)` *(property)* — The rows as a :class:`~uitk.managers.field_visibility.FieldVisibility`.
  - `WidgetComboBox.host_of(cls, widget) -> Optional['WidgetComboBox']` *(class)* — The combo whose row embeds *widget*, or ``None``.
  ...
  - `WidgetComboBox.showPopup(self) -> None` — Override to expand the popup to the widest embedded widget.
