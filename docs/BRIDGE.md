...
- `DOCS_URL` / `DOCS_LABEL` (or the `docs_url()` hook) — the panel's documentation link, logged once at open as a clickable anchor in the log pane (see [Panel services](#panel-services)); empty (the default) logs nothing.

### End to end

Init order in `__init__(switchboard)`: Output Dir row (if `REQUIRE_OUTPUT_DIR`) → parameter widgets → `action`-button wiring → preset controls → enablement-refresh wiring → log redirect + `anchorClicked` wiring → `STARTUP_INFO` → docs link (`DOCS_URL`). `cmb000_init` then populates the template combo (and builds its context menu) and fires the first `_on_template_changed`, which shows only the rows the template references, re-applies row enablement, re-points the preset dir (widget-state mode only), and logs the template's description.

A concrete walk-through, from `mayatk.env_utils.blender_bridge` (Maya → Blender hand-off): `BlenderBridgeSlots` subclasses `MayaBridgeSlotsBase` (itself a thin `BridgeSlotsBase` subclass adding the Maya output-dir fallback and `SCOPE` resolution), sets `UI_NAME` / `PRESETS_ROOT` / `LOG_TAG` / `REQUIRE_OUTPUT_DIR = False` / `HELP_SPEC`, points `params_module` at its `parameters.py` and `template_dir` at its `templates/` folder, and implements `b000` — which collects the parameter values, resolves the export set from the shared `SCOPE` parameter via `scoped_objects()`, reads the active `(template, mode)` pair via `_selected_template_mode()`, and calls:

```python
# mayatk.env_utils.blender_bridge.blender_bridge_slots — b000 (abridged)
params = self.collect_param_values()
selection = self.scoped_objects(params)   # SCOPE → Maya objects; logs when empty
template, mode = self._selected_template_mode()
with self.sb.progress(text=f"Working: Send to Blender ({template})"):
    self.bridge.send(
        objects=selection, template=template, mode=mode, params=params
    )
```

`collect_param_values()` snapshots every parameter widget (visible or not) through the kind registry — except `action` rows, which carry no value; the transport renders the chosen template with those values substituted and launches a fresh Blender on the result.
...
| `action` | composite: one `QPushButton` per `choices` entry (`(label, action_id[, tooltip])`; the first takes the row's stretch) | none — `read` returns `None`, excluded from `collect_param_values` | deliberate no-op (clicks never dirty presets) |

An `action` row's "value" is a set of commands, not data: `BridgeSlotsBase._wire_action_params` connects each `action_id` to the same-named method on the slot class, so a registry declares panel actions as data with zero per-panel wiring; an id with no matching method is disabled with an explanatory tooltip.
...
Each formatter is a `Formatters` staticmethod in [`formatters.py`](../uitk/bridge/formatters.py) with the signature `formatter(spec, value) -> str`, rendering a value as a literal in one target language. Decoupling formatters from the spec lets one `AttributeSpec` serve any number of target languages — the bridge author picks a formatter at render time. All four share one float renderer that formats to `spec.decimals` and strips trailing zeros (plain `repr` when `decimals` is unset).
...
`list_template_modes()` is a subclass hook returning `[(stem, mode), ...]`; the combo shows each pair as `"<template> (<mode>)"`, with the parens elided when `mode` is `""` (single-mode bridges like rizom). `select_initial_template_index` picks the starting entry. Template management lives on the combo itself: its context menu (built in `cmb000_init`) carries **Refresh Templates** / **Open Templates Folder**, backed by `refresh_templates()` / `open_templates_folder()`.
...
**Header menu** — the default `header_init` builds a titled separator (`HEADER_MENU_TITLE`, default "Utilities"), one button per `HEADER_MENU_ITEMS` entry — `(label, objectName, tooltip, handler_method_name)` tuples, each handler resolved on the slot via `getattr` — and the rich-text help from `HELP_SPEC` (a keyword dict for the tooltip-mixin `fmt()`: `title` / `body` / `bullets` / `steps` / `rows` / `sections` / `notes`). The stock item set is just Clear Log (backed by `clear_log`) — template actions live on the template combo's context menu, not the header. `open_templates_folder` / `refresh_templates` and the shared `reveal_folder` helper are provided for subclass menus to name. Subclasses customise by setting data (or the `header_menu_items()` / `help_spec()` hooks), not code; overriding `header_init` wholesale remains possible.
...
**Parameter tooltips** — each row's label and widget get `format_param_tooltip(spec)`: rich-text HTML with the spec's title and body plus Type / Range / Step / Default rows and, for the choice-driven kinds (`choice` / `check_list`), a bullet list of options (`\n` in registry tooltips becomes `<br>` so hand-wrapped text keeps its line breaks).
