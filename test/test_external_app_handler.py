...
class TestExternalAppVisibilityTracking(unittest.TestCase):
    ...
    def test_event_filter_installed_on_show(self):
        ...
        h.register(
            "vt",
            module="fake_external_app_visibility",
            entry="FakeUI",
            mode="in_process",
        )
    ...
    def test_widget_hide_fires_entry_changed_signal(self):
        ...
        received = []
        sb.on_handler_entry_changed.connect(lambda hn, en: received.append((hn, en)))
        with patch.object(ExternalAppHandler, "_is_importable", return_value=True):
            widget = h.launch("vt")
        ...
        self.assertTrue(
            ...
            f"widget.hide() must fire on_handler_entry_changed; received={received}",
        )
        ...
        with patch.object(ExternalAppHandler, "_is_importable", return_value=True):
            ...
            len(
                widget.findChildren(
                    ...
                    if False
                    else type(widget.parent() or widget)
                )
            )
            h.launch("vt")
...
class TestExternalAppDiscovery(unittest.TestCase):
    ...
    def test_manual_registration_overrides_discovered_mode(self):
        ...
        h.register(
            "flexible_tool", module="flexible_tool", entry="UI", mode="in_process"
        )
    ...
    def test_entries_surface_discovered_tools(self):
        ...
        eps = [
            _FakeEP("alpha", "alpha", "AlphaUI", ["x"]),
            _FakeEP("beta", "beta", "BetaUI", ["y"]),
        ]
        with self._patched_eps(inproc=eps):
            sb = _make_sb()
        names = {
            e.name for e in sb.iter_handler_entries() if e.kind.startswith("external")
        }
...
class TestExternalAppContextVisibility(unittest.TestCase):
    ...
    def test_gated_app_still_launchable_by_name(self):
        ...
        self._context({"maya"})
        self.h.register(
            "substance", module="m2", entry="UI2", hidden_in={"maya"}, mode="subprocess"
        )
        self.assertNotIn("substance", {e.name for e in self.h.entries()})
        with (
            patch.object(ExternalAppHandler, "_is_importable", return_value=True),
...
class TestProviderBootstrap(unittest.TestCase):
    ...
    def test_provider_attempted_only_once_per_session(self):
        ...
        with (
            ...
            patch.object(ExternalAppHandler, "discover", lambda self_, groups=None: 0),
            patch("pythontk.PackageManager", return_value=pm_instance),
        ):
            with self.assertRaises(ValueError):
                h.launch("ghost")
    ...
    def test_present_provider_is_not_reinstalled(self):
        ...
        def fake_discover(self_, groups=None):
            self_.register(
                "compositor", module="extapps.c", entry="UI", mode="subprocess"
            )
            return 1
...
class TestExternalAppHandlerRegistry(unittest.TestCase):
    ...
    def test_singleton_key_does_not_collide_with_other_handlers(self):
        ...
        sb = Switchboard(handlers={"ui": UiHandler, "external_app": ExternalAppHandler})
...
class TestExternalAppHandlerLaunch(unittest.TestCase):
    ...
    def test_importable_module_skips_install(self):
        ...
        with (
            ...
            patch("pythontk.PackageManager") as pm,
            patch.object(
    ...
    def test_missing_module_triggers_install(self):
        ...
        with (
            ...
            self.handler.launch(module="anymod", entry="UI", install_spec="anymod")
    ...
    def test_missing_module_without_install_spec_raises(self):
        with patch.object(ExternalAppHandler, "_is_importable", return_value=False):
            with self.assertRaises(RuntimeError):
    ...
    def test_self_heal_preserves_install_augmentation_fields(self):
        ...
        with (
            patch("importlib.metadata.entry_points", side_effect=fake_eps),
            patch.object(
                ExternalAppHandler,
                "_is_importable",
...
class TestInProcessMode(unittest.TestCase):
    ...
    def test_in_process_returns_widget_from_entry(self):
        ...
        with (
            patch.object(ExternalAppHandler, "_is_importable", return_value=True),
            patch("importlib.import_module", return_value=fake_mod) as import_mod,
        ):
    ...
    def test_in_process_caches_widget_by_name(self):
        ...
        self.handler.register("tool", module="m", entry="E", mode="in_process")
...
class TestInProcessShowOptOut(unittest.TestCase):
    ...
    def test_show_false_does_not_show_widget(self):
        ...
        h.register(
            "so",
            module="fake_external_app_show_optout",
            entry="FakeUI",
            mode="in_process",
        )
        with patch.object(ExternalAppHandler, "_is_importable", return_value=True):
...
class TestInstallFailureSurfaces(unittest.TestCase):
    ...
    def test_pm_install_exception_becomes_runtime_error(self):
        ...
        with (
            ...
            with self.assertRaises(RuntimeError) as ctx:
                h.launch(module="missing", entry="UI", install_spec="missing")
...
class TestDefaultPython(unittest.TestCase):
    ...
    def test_last_resort_returns_sys_executable(self):
        ...
        with (
            patch.object(eth.shutil, "which", return_value=None),
            patch.object(eth.sys, "executable", fake_exe),
        ):
            self.assertEqual(eth.ExternalAppHandler._default_python(), fake_exe)
...
class TestIsImportable(unittest.TestCase):
    def test_same_interpreter_uses_find_spec(self):
        ...
        self.assertTrue(ExternalAppHandler._is_importable("sys", sys.executable))
        self.assertFalse(
    ...
    def test_different_interpreter_runs_probe(self):
        ...
        with patch("uitk.handlers.external_app_handler.subprocess.run") as run:
